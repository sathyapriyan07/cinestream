...
export default function Series() {
  ...
  useEffect(() => {
    tmdb.genres('tv').then(d => setGenres(d.genres)).catch(() => {})
  }, [])
  ...
  useEffect(() => {
    ...
    supabase.from('series').select('id', { count: 'exact', head: true }).then(async ({ count }) => {
      if (count && count > 0) {
        setFromSupabase(true)
        const { data, count: total } = await loadFromSupabase()
        setShows(data.map(normalizeSeries))
